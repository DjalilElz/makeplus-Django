...
from .models_eposter import EPosterSubmission, EPosterCommitteeMember
...
def eposter_enable_for_event(request, event_id):
    """
    Enable ePoster/Appel à Communication for an event
    """
    event = get_object_or_404(Event, id=event_id)
    
    if request.method == 'POST':
        # You can add event-specific configuration here if needed
        # For now, ePoster is automatically available for all events
        messages.success(
            request, 
            f'Appel à Communication activé pour "{event.name}". '
            f'URL publique: /eposter/{event.id}/'
        )
