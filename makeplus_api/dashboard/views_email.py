...
def email_template_list(request):
    ...
    from .models_email import EmailTemplate, EmailCampaign
    
    # Get all templates
    templates = EmailTemplate.objects.filter(is_active=True).select_related('created_by').order_by('-created_at')
...
def registration_form_builder(request):
    ...
    # Close connection to ensure fresh data
    connection.close()
    
    # Get forms with actual submission count from database
    forms = FormConfiguration.objects.select_related('event', 'created_by').annotate(
        actual_submission_count=Count('submissions')
    ).order_by('-created_at')
    ...
    response = render(request, 'dashboard/registration_form_list.html', context)
...
def registration_form_create(request):
    ...
    if request.method == 'POST':
        ...
        # Handle email template if selected
    ...
    # Get events that DON'T have a form yet (one-to-one relationship)
    events_with_forms = FormConfiguration.objects.values_list('event_id', flat=True)
    events = Event.objects.exclude(id__in=events_with_forms).order_by('-created_at')
    ...
    # Prevent caching
    response = render(request, 'dashboard/registration_form_builder.html', context)
    response['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    response['Pragma'] = 'no-cache'
    response['Expires'] = '0'
    return response
...
def registration_form_edit(request, form_id):
    ...
    # Get events that DON'T have a form, PLUS the current form's event
    # (so user can keep the same event or change to an available one)
    events_with_forms = FormConfiguration.objects.exclude(id=form.id).values_list('event_id', flat=True)
    events = Event.objects.exclude(id__in=events_with_forms).order_by('-created_at')
...
@login_required
def registration_form_toggle(request, form_id):
    """Toggle form active/inactive status"""
    from .models_form import FormConfiguration
    from django.db import connection
    
    if request.method == 'POST':
        # Close any database connections to ensure fresh data
        connection.close()
        
        form = get_object_or_404(FormConfiguration, id=form_id)
        
        # Debug logging
        print(f"DEBUG TOGGLE: Form '{form.name}' current status: is_active={form.is_active}")
        ...
        cache.clear()
        print(f"DEBUG TOGGLE: All caches cleared")
...
def registration_form_submissions(request, form_id):
    ...
    not_participated_count = registered_count - participated_count
    
    # Prepare submissions as JSON for JavaScript with participation status
    submissions_list = []
    for sub in submissions:
        is_participated = sub.email in participants_checked_in
    ...
    context = {
        ...
        'participants_checked_in': list(participants_checked_in),
    }
...
def campaign_create(request):
    ...
    # Get all events with their registration fields for pre-loading
    from .models_form import FormConfiguration
    events_with_fields = {}
    for event in events:
        custom_form = FormConfiguration.objects.filter(event=event, is_active=True).first()
        fields = []
        
        if custom_form and custom_form.fields_config:
            # Use fields from the custom form builder
            for field_config in custom_form.fields_config:
            ...
            # Fallback to base EventRegistration fields
            fields = [
                {'name': 'nom', 'label': 'Nom (Last Name)'},
                {'name': 'prenom', 'label': 'Prénom (First Name)'},
        ...
        events_with_fields[str(event.id)] = fields
    ...
    context = {
        'form': form,
        'events': events,
        'events_with_fields': json.dumps(events_with_fields),  # Convert to JSON for JavaScript
        'template_variables': template_variables,
...
def campaign_delete(request, campaign_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Delete all related recipients first (cascade should handle this, but let's be explicit)
            recipient_count = campaign.recipients.count()
            campaign.recipients.all().delete()
            
            logger.info(f"Deleted {recipient_count} recipients")
            
            # Delete all related links
            link_count = campaign.tracked_links.count()
            campaign.tracked_links.all().delete()
            
            logger.info(f"Deleted {link_count} tracked links")
...
@login_required
def campaign_import_form_submissions(request, campaign_id):
    """Import form submissions from event registration form as campaign recipients"""
    from .models_email import EmailCampaign, EmailRecipient
    from .models_form import FormConfiguration, FormSubmission
    import secrets
...
def campaign_send_test(request, campaign_id):
    """Send a test email for a campaign using configured SMTP backend."""
    from .models_email import EmailCampaign, EmailRecipient
    ...
    if request.method == 'POST':
        try:
            ...
            # Create a temporary recipient object to use build_recipient_context
            temp_recipient = EmailRecipient(
                campaign=campaign,
                email=test_email,
                name='Test User'
            )
            
            # Build context with real form data if available
            context = build_recipient_context(temp_recipient, campaign)
            
            # Replace template variables using utils function
            ...
            if success:
                response_data = {'success': True, 'message': f'Test email sent to {test_email}.'}
                if message_id:
                    response_data['message_id'] = message_id
                    response_data['message'] += ' (Tracking enabled)'
...
@login_required
def get_event_registration_fields(request, event_id):
    """
    API endpoint to get registration form fields for a specific event
    Returns the field names that can be used as variables in email templates
    """
    from django.http import JsonResponse
    ...
    try:
        ...
        # Try to find a custom registration form for this event
        custom_form = FormConfiguration.objects.filter(event=event, is_active=True).first()
        ...
        if custom_form and custom_form.fields_config:
            ...
            for field_config in custom_form.fields_config:
                if isinstance(field_config, dict):
                    ...
                    if field_name:  # Only add if field has a name
                        all_fields.append({
                            'name': field_name,
                            'label': field_label,
                            'type': field_type,
            ...
            # Fallback to base EventRegistration fields if no custom form exists
            all_fields = [
                {'name': 'nom', 'label': 'Nom (Last Name)', 'type': 'text'},
                {'name': 'prenom', 'label': 'Prénom (First Name)', 'type': 'text'},
                {'name': 'email', 'label': 'Email', 'type': 'email'},
                {'name': 'telephone', 'label': 'Téléphone', 'type': 'text'},
                {'name': 'pays', 'label': 'Pays', 'type': 'text'},
                {'name': 'wilaya', 'label': 'Wilaya', 'type': 'text'},
        ...
        return JsonResponse({
            ...
            'event_name': event.name,
            'has_custom_form': custom_form is not None,
            'form_name': custom_form.name if custom_form else None,
...
@login_required
def event_registration_fields_api(request, event_id):
    """
    API endpoint to get registration fields for an event
    Used for dynamic variable loading in campaign form
    """
    from django.http import JsonResponse
    from .models_form import FormConfiguration
    ...
    try:
        ...
        form_config = FormConfiguration.objects.filter(event=event, is_active=True).first()
        
        fields = []
        if form_config and form_config.fields_config:
            for idx, field_config in enumerate(form_config.fields_config):
                if isinstance(field_config, dict):
                    field_name = field_config.get('name', '')
                    field_label = field_config.get('label', field_name.replace('_', ' ').title())
                    field_type = field_config.get('type', 'text')
