...
class PasswordResetVerification(models.Model):
    """
    Email verification codes for self-service password reset
    Expires after 3 minutes. Same shape as SignUpVerification, with
    signup_data replaced by a single pre-hashed new_password_hash.
    """
    email = models.EmailField(db_index=True)
    code_hash = models.CharField(max_length=64, db_index=True)

    # New password, already hashed -- applied only once the code is verified
    ...
    @classmethod
    def create_verification(cls, email, new_password_hash, ip_address=None, user_agent=''):
        """Create a new verification code"""
        code = cls.generate_code()
        expires_at = timezone.now() + timedelta(minutes=3)
