...
class LoginCodeTokenObtainView(APIView):
    """
    JWT Token authentication using email + login code (passwordless)
    For mobile app participants who register via form
    """
    permission_classes = []
    ...
    def post(self, request):
        ...
        if not email or not code or not event_id:
            return Response({
                'detail': 'Email, code, and event_id are required'
        ...
        # Verify login code
        from events.login_code_service import verify_login_code, mark_code_as_used
